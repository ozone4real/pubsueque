...
```ruby
{
    concurrency: 4 # number of threads that each pub/sub subscription would listen for and process published messages(jobs). It is best to keep this not-too-high, so as to work well with ActiveRecord's connection pool.
